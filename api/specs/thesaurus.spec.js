...
const endpoints = require("../endpoints/epThesaurus.js");
const tests = require("../commonsTestsAPI");
const commons = require("../commons");
const url = commons.baseAPI.urlAutentication;
const HTTPStatusCodes = require("../httpStatusCodes");
const getEndpoints = endpoints.endpointsThesaurus.filter((e) => e.method === "get");
const putEndpoints = endpoints.endpointsThesaurus.filter((e) => e.method === "put");
const postEndpoints = endpoints.endpointsThesaurus.filter((e) => e.method === "post");
let tok = "";

describe("Testando APIs de Thesaurus ", () => {
    before((done) => {
    ...
    describe("Realizando Health Tests - 200", () => {
        ...
        for (let val of postEndpoints) {
            it(val.number + ") - (POST) API: " + val.name + " - 204", async () => {
    ...
    describe("Realizando teste unauthorized - 401", () => {
        ...
        for (let val of postEndpoints) {
            it(val.number + ") - (POST) API: " + val.name + " - 401", async () => {
                const res = await tests.testUnauthorized(url, val.path + val.parameters);
                expect(res.type).to.be.equal("application/json");
                expect(res.status).to.be.equal(HTTPStatusCodes.FORBIDDEN);
            });
        }
    });
