...
var until = p.ExpectedConditions
var fs = require('fs')
...
Helper.prototype.elementIsPresenceDom = function (element) {
  p.browser.wait(until.presenceOf(element), 25000, 'Element ' + element.getText() + ' taking too long to appear in the DOM')
  p.browser.executeScript('arguments[0].scrollIntoView();', element.getWebElement())
}

// Wait to see if element is clickable
Helper.prototype.elementIsClickable = function (element) {
  p.browser.wait(until.elementToBeClickable(element), 50000, 'Element taking too long to appear in the DOM and stay clickable')
}

// Wait to see if element is visible
Helper.prototype.elementIsVisible = function (element) {
  p.browser.wait(until.visibilityOf(element), 20000, 'Element taking too long to appear in the DOM and stay visible')
}

// Wait to see if element is not attache to the DOM
Helper.prototype.elementIsNotAttachedOnDom = function (element) {
  p.browser.wait(until.stalenessOf(element), 10000, 'Element appeared in DOM')
}

// Wait to see if element is not present of DOM
Helper.prototype.elementIsNotPresentOfDom = function (element) {
  return p.browser.wait(until.not(until.presenceOf(element)))
}

// Force the browser to stop
Helper.prototype.stopBrowser = function (time) {
  p.browser.sleep(time)
}
...
Helper.prototype.getScreenshot = function (nomeArquivo) {
  ...
  return p.browser.takeScreenshot().then((png) => {
...
Helper.prototype.scrollPageDown = function (valuePixels) {
  p.browser.executeScript('window.scrollBy(0,' + valuePixels + ');')
}
